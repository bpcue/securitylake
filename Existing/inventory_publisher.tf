...
resource "aws_cloudwatch_log_group" "ou_inventory_lambda" {
  provider          = aws.security_lake
  name              = "/aws/lambda/${local.ou_inventory_lambda_name}-existing"
  retention_in_days = 30
}

resource "aws_lambda_function" "ou_inventory" {
  provider      = aws.security_lake
  function_name = "${local.ou_inventory_lambda_name}-existing"
...
resource "aws_cloudwatch_event_rule" "ou_inventory_org_events" {
  provider    = aws.security_lake
  name        = "securitylake-ou-sync-existing-events"
  description = "Sync OU inventory for existing deployment when accounts move."
...
resource "aws_lambda_permission" "allow_eventbridge_org" {
  provider     = aws.security_lake
  statement_id = "AllowExecutionFromEventBridgeOrgExisting"
...
resource "aws_cloudwatch_event_rule" "ou_inventory_scheduled" {
  provider            = aws.security_lake
  name                = "securitylake-ou-sync-existing-daily"
  description         = "Daily OU inventory refresh for existing deployment."
...
resource "aws_lambda_permission" "allow_eventbridge_schedule" {
  provider      = aws.security_lake
  statement_id  = "AllowExecutionFromEventBridgeScheduleExisting"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.ou_inventory.function_name
  principal     = "events.amazonaws.com"
